...
class TSPModel(nn.Module):
    ...
    def pre_forward(self, reset_state):
        ...
        radius = reset_state.radius
        self.encoded_nodes = self.encoder(node_xy, radius)
        # shape: (batch, problem, EMBEDDING_DIM)
...
class TSP_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.embedding_radius = nn.Linear(1, embedding_dim)
        self.embedding_all = nn.Linear(2 * embedding_dim, embedding_dim)
        self.layers = nn.ModuleList([EncoderLayer(**model_params) for _ in range(encoder_layer_num)])

    def forward(self, node_xy, radius):
        # data.shape: (batch, problem, 2)

        embedded_node = self.embedding_node(node_xy)
        embedded_radius = self.embedding_radius(radius[:, :, None])
        # shape: (batch, problem, embedding)

        out = self.embedding_all(torch.cat((embedded_node, embedded_radius), -1))
